 ...
 *
 * v7 adds:
 *   - Closes a lead that Adversus no longer lists as open.
 *
 *     A lead dropped from a campaign keeps its last outcome, so one sitting in
 *     "Automatic redial" stayed open forever and could not be closed by hand:
 *     this script rewrites Lead open/closed on every run and overwrote the edit
 *     within about 40 seconds. Roughly 2,800 leads were open for that reason.
 *
 *     The campaign export is the only thing that knows a lead has been dropped,
 *     so it is loaded into a field this script reads instead of one it writes.
 *     Populate ADVERSUS_OPEN_LIST with tools/airtable_close_leads.py.
 *
...
function buildStatusUpdate(record, meta) {
    ...
    const openList = FIELD.ADVERSUS_OPEN_LIST
        ? norm(record.getCellValue(FIELD.ADVERSUS_OPEN_LIST))
        : '';
    const exportLoaded = openList === 'yes' || openList === 'no';
    const notInAdversus = openList === 'no';
    ...
    const targetCalledId = timesCalled > 0
        ? meta.yesChoiceId
        : meta.noChoiceId;
    const targetOpenClosed = computeOpenClosed(adversusText, timesCalled, notInAdversus, exportLoaded);
    ...
    return {
        ...
        warnings: [
            ...
            adversusText && !adversusSelectId
                ...
                : null,
            !FIELD.ADVERSUS_OPEN_LIST
                ? 'FIELD.ADVERSUS_OPEN_LIST is not set, so leads dropped from a campaign stay open'
                : null,
            FIELD.ADVERSUS_OPEN_LIST && !exportLoaded
                ? 'This lead has never been checked against an Adversus export'
                : null,
        ].filter(Boolean),
    };
}
...
if (FIELD.ADVERSUS_OPEN_LIST) {
    loadFields.push(table.getField(FIELD.ADVERSUS_OPEN_LIST));
}
...
output.set('result', {
    ...
    adversusOpenList: result.openList,
